...
class FormularioCita(tk.Toplevel):
    ...
    def _configure_styles(self):
        ...
        self.configure(background="#B2D9C4") 
        style.configure("Form.TFrame", background="#B2D9C4")
        
        # Estilo para etiquetas y campos
        style.configure("TLabel", background="#B2D9C4", font=('Arial', 10, 'bold'))
    ...
    def _load_doctors(self):
        ...
        self.doctor_map = {f"{d['Nombre_usuario']} (ID:{d['ID_Usuario']})": d['ID_Usuario'] for d in doctors}
    ...
    def _create_widgets(self):
        main_frame = ttk.Frame(self, padding="15", style="Form.TFrame")
        ...
        ttk.Label(main_frame, text="Paciente:", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        self.search_entry = ttk.Entry(main_frame, textvariable=self.search_var)
        self.search_entry.grid(row=row, column=1, sticky="ew", pady=5, padx=5)
        self.search_button = ttk.Button(main_frame, text="Buscar", command=self._search_paciente)
        self.search_button.grid(row=row, column=2, sticky="e", pady=5, padx=5)
        row += 1
        
        # --- Info ID Paciente ---
        self.paciente_info_label = ttk.Label(main_frame, text="ID: (No seleccionado)", anchor='w', font=('Arial', 9))
        self.paciente_info_label.grid(row=row, column=0, columnspan=3, sticky="w", padx=5)
        row += 1

        # --- Doctor Asignado ---
        ttk.Label(main_frame, text="Doctor Asignado:", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        self.doctor_combobox = ttk.Combobox(main_frame, textvariable=self.selected_doctor_id, values=self.doctor_options, state="readonly")
        self.doctor_combobox.grid(row=row, column=1, columnspan=2, sticky="ew", pady=5, padx=5)
        row += 1

        # --- Fecha ---
        ttk.Label(main_frame, text="Fecha (YYYY-MM-DD):", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        ttk.Entry(main_frame, textvariable=self.date_var).grid(row=row, column=1, columnspan=2, sticky="ew", pady=5, padx=5)
        row += 1

        # --- Hora ---
        ttk.Label(main_frame, text="Hora (HH:MM:SS):", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        ttk.Entry(main_frame, textvariable=self.time_var).grid(row=row, column=1, columnspan=2, sticky="ew", pady=5, padx=5)
        row += 1

        # --- Motivo ---
        ttk.Label(main_frame, text="Motivo:", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        ttk.Entry(main_frame, textvariable=self.motivo_var).grid(row=row, column=1, columnspan=2, sticky="ew", pady=5, padx=5)
        row += 1

        # --- Estado (Solo para Modificación) ---
        ttk.Label(main_frame, text="Estado:", anchor='w').grid(row=row, column=0, sticky="w", pady=5, padx=5)
        estado_options = ["Agendada", "Cancelada", "Completada"]
        self.estado_combobox = ttk.Combobox(main_frame, textvariable=self.estado_var, values=estado_options, state="readonly")
        self.estado_combobox.grid(row=row, column=1, columnspan=2, sticky="ew", pady=5, padx=5)
        row += 1
        ...
        ttk.Button(main_frame, text=button_text, command=command, style="Guardar.TButton").grid(
    ...
    def _search_paciente(self):
        """Busca pacientes y permite seleccionar uno."""
        search_term = self.search_var.get()
        if not search_term:
            messagebox.showwarning("Advertencia", "Ingrese un nombre o apellido para buscar.")
            return

        pacientes = self.cita_controller.search_pacientes(search_term)

        if not pacientes:
            messagebox.showinfo("Búsqueda", "No se encontraron pacientes con ese criterio.")
            self.paciente_id = None
            self.paciente_info_label.config(text="ID: (No seleccionado)")
            return

        # Simple selección del primer resultado (En un sistema real, se usaría un ComboBox o lista)
        selected_paciente = pacientes[0] 
        self.paciente_id = selected_paciente['ID_Paciente']
        full_name = f"{selected_paciente['Nombres']} {selected_paciente['Apellidos']}"
